...
TEST(movie_scene, spawn_fn_creates_objects)
{
	...
	portal::info l_portal_info;
	camera::info l_camera_info;

	// when
	uint l_sector_ID = l_scene.spawn(l_sector_info);
	uint l_portal_ID = l_scene.spawn(l_portal_info, l_sector_ID, r4x4_1);
	uint l_camera_ID = l_scene.spawn(l_camera_info, l_sector_ID, r4x4_1);
